<div id="header" class="header" data-fixed-top="true">

<div class="container">

    <div class="header-container">

        <button type="button" class="navbar-toggle collapsed" data-bs-toggle="collapse"
            data-bs-target="#navbar-collapse">
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
        </button>


        <div class="header-logo">
            <a href="index.html">
                <img src="assets/img/lostcard.png" alt="">
            </a>
        </div>


        <div class="header-nav">
            <div class="collapse navbar-collapse" id="navbar-collapse">
                <ul class="nav justify-content-center">
                    <li class=""><a href="index.php">Accueil</a></li>

                    <li><a href="recherche.php">Pièces trouvés</a></li>

                    <li><a href="declaration.php">Déclaration de perte</a></li>

                    <li class="dropdown dropdown-hover">
                        <a href="#" data-bs-toggle="dropdown">
                            Lange
                            <b class="caret"></b>
                            <span class="arrow top"></span>
                        </a>
                        <div class="dropdown-menu">
                            <a href="#" class="dropdown-item"><img src="assets/img/flag-up-1.png" height="30px" width="30px" alt="" />  English</a>
                            <a href="#" class="dropdown-item"><img src="assets/img/flag-up-2.png" height="30px" width="30px"  alt="" />  Français</a>
                        </div>
                    </li>

                </ul>
            </div>
        </div>


        <div class="header-nav">
            <ul class="nav justify-content-end">
                <li class="dropdown dropdown-hover">
                    <a href="#" class="header-cart" data-bs-toggle="dropdown">
                        <i class="fa fa-user"></i>
                        <span class="total" style="background-color:#0b9444;"><b class="caret"></b></span>
                        <span class="arrow top"></span>
                    </a>
                    <div class="dropdown-menu dropdown-menu-cart p-0">

                        <div class="cart-footer">
                            <div class="row gx-2">
                                <div class="col-6">
                                    <a href="checkout_cart.html"
                                        class="btn btn-dark" style="background-color:grey;">Connexion</a>
                                </div>
                                <div class="col-6">
                                    <a href="checkout_cart.html" class="btn btn-dark" style="background-color:#0b9444;">Inscription</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </li>
                <li class="divider"></li>
                <li>
                    <a href="compte.php">
                        <img src="assets/img/user-icon2.jpg" class="user-img" alt="" />
                        <span class="d-none d-xl-inline">Name_user</span>
                    </a>
                </li>
            </ul>
        </div>

    </div>

</div>

</div>